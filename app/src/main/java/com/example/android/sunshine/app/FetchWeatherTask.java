...
//    }
    ...
    long addLocation(String locationSetting, String cityName, double lat, double lon) {
        long locationID = -1; // the internal database row id, not the area code location id

        // Students: First, check if the location with this city name exists in the db
        Cursor cursor = mContext.getContentResolver().query(
                WeatherContract.LocationEntry.CONTENT_URI,
                new String[]{WeatherContract.LocationEntry._ID},   // projection return one column; leaving blank for all columns not necessar
                WeatherContract.LocationEntry.COLUMN_LOCATION_SETTING + " = ?", // query "where" clause on location setting, instead of location ID.
    ...
    private void getWeatherDataFromJson(String forecastJsonStr,
        ...
        try {
            ...
            if ( cVVector.size() > 0 ) {
                ...
                inserted = mContext.getContentResolver().bulkInsert(WeatherEntry.CONTENT_URI,weatherValues);
            }
//
